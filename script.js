...
// ปุ่ม Yes
document.getElementById('btnYes').addEventListener('click', () => { showPage('page2'); });
...
btnNo.addEventListener('click', () => { noClickCount++; if(noClickCount>=1) showPage('pageNo'); });

// กลับไปหน้าแรก
document.getElementById('btnBackToStart').addEventListener('click', () => {
  btnNo.style.position='relative'; btnNo.style.left='0'; btnNo.style.top='0'; btnNo.style.zIndex='auto';
  noClickCount=0; showPage('page1');
});

// Next button
document.getElementById('btnNext').addEventListener('click', () => { showPage('page3'); });
...
document.getElementById('btnSubmitPassword').addEventListener('click', () => {
  ...
  if(password==='L0VEY0U') { failMessage.style.display='none'; document.getElementById('passwordInput').value=''; showPage('page4'); }
  else { failGif.src=failGifs[Math.floor(Math.random()*failGifs.length)]; failMessage.style.display='block'; }
});
document.getElementById('passwordInput').addEventListener('keypress', e=>{ if(e.key==='Enter') document.getElementById('btnSubmitPassword').click(); });

// Activity click
document.querySelectorAll('.activity-box').forEach(box=>{
  box.addEventListener('click', ()=>{ showPage('activity'+box.getAttribute('data-activity')); });
});

// Back buttons
['btnBack1','btnBack2','btnBack3'].forEach(id=>{ document.getElementById(id).addEventListener('click',()=>{ showPage('page4'); }); });
